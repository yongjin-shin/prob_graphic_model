...
class VDPGMM(object):
    ...
    def _initialize(self, X):
        ...
        self.clusters = np.argmax(self.phi, axis=0)
    ...
    def predict(self, X):
        ...
        self.clusters = np.argmax(phi, axis=0)


def test1():
    print('test1')
    model = VDPGMM(T=10, alpha=1, max_iter=100)
...
# def _main():
#     model = VDPGMM(T=10, alpha=100, max_iter=50)
#     df = pd.read_csv('faithful.csv')
#     X = df.values
#     model.fit(X)
#     y = model.predict(X)
